...
Overall, XGBoost achieves the lowest CV RMSE of 12.43 cm and is therefore selected as the final model. On the sealed test set, XGBoost achieves an RMSE of 13.90 cm, while the MLP performs better with an RMSE of 12.11 cm. The addition of upstream stations and weather data leads to a decrease in CV RMSE of 25.8% compared to using only target-station features without weather data. The persistence model performs better for the first five hours, with XGBoost performing better for the remaining forecast horizon. However, XGBoost struggles to predict extreme water levels accurately. At or above the alarm threshold of 545 cm, the RMSE increases to 71.44 cm, with the model systematically underestimating the water level.
